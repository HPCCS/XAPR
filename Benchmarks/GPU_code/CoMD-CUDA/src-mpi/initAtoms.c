...
Atoms* initAtoms(LinkCell* boxes, const real_t skinDistance)
{
   ...
   int iOff;
   for (iOff = 0; iOff < maxTotalAtoms; iOff++)
   {
      atoms->gid[iOff] = 0;
...
void createFccLattice(int nx, int ny, int nz, real_t lat, SimFlat* s)
{
   ...
   real_t px,py,pz;
   px=py=pz=0.0;
   for (ix=begin[0]; ix<end[0]; ++ix)
      for (iy=begin[1]; iy<end[1]; ++iy)
         for (iz=begin[2]; iz<end[2]; ++iz)
            for (ib=0; ib<nb; ++ib)
            {
...
void setVcm(SimFlat* s, real_t newVcm[3])
{
   ...
   {
      for (iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
...
{
   int iBox,iOff,ii, count = 0;
   for (iBox=0; iBox<boxes->nLocalBoxes; ++iBox)
   {
      for (iOff=MAXATOMS*iBox, ii=0; ii<boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
