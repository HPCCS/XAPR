...
LinkCell* initLinkCells(const Domain* domain, real_t cutoff, int useHilbert)
{
   ...
   for(i = 0; i < ll->gridSize[2]; ++i){
      for(j = 0; j < ll->gridSize[1]; ++j){
         for(k = 0; k < ll->gridSize[0]; ++k){
   ...
   int iBox;
   for (iBox=0; iBox<ll->nTotalBoxes; ++iBox)
      ll->nAtoms[iBox] = 0;
...
{
   int ii,localMax = 0;
   for (ii=0; ii<boxes->nLocalBoxes; ++ii)
...
{
   int ii;
   for (ii=boxes->nLocalBoxes; ii<boxes->nTotalBoxes; ++ii)
      boxes->nAtoms[ii] = 0;
}
...
{
      int ii,i; 
      for(i = 0; i < sim->n_boundary1_cells; ++i)
      {
              ...
              for(ii=0; ii<sim->boxes->nAtoms[iBox];++ii)
              {
